    ...
    try {
        ...
        if (category) {
            ...
            query += ` AND (
                ...
                OR LOWER(REPLACE(c.name, '-', ' ')) = LOWER($${paramCount})
                OR REPLACE(REPLACE(LOWER(c.name), 'concern', 'care'), ' ', '-') = LOWER($${paramCount})
                OR REPLACE(REPLACE(LOWER(c.name), 'care', 'concern'), ' ', '-') = LOWER($${paramCount})
                ...
                OR EXISTS (
                    ...
                    AND (
                        ...
                        OR LOWER(REPLACE(c_join.name, '-', ' ')) = LOWER($${paramCount})
                        OR REPLACE(REPLACE(LOWER(c_join.name), 'concern', 'care'), ' ', '-') = LOWER($${paramCount})
                        OR REPLACE(REPLACE(LOWER(c_join.name), 'care', 'concern'), ' ', '-') = LOWER($${paramCount})
                    )
                )
            )`;
